...
public final class DateUtil {
    ...
    public static void main(String args[]) {
        System.out.println(date2long("2017-03-02 00:00:00"));
    }
    ...
    public static Long date2long(String str) {
        Date date = parse(str, PATTERN_CLASSICAL);
    ...
    public static String getWeekdayDesc(Date date) {
        final String[] weeks = new String[]{"周日", "周一", "周二", "周三", "周四",
                "周五", "周六"};
        Calendar cal = Calendar.getInstance();
        if (date != null) {
            cal.setTime(date);
...
//            return Math.abs(seconds) + "秒" + (seconds > 0 ? "前" : "后");
    ...
    public static int secondToMinute(int second) {
        return Double.valueOf(Math.ceil(second / 60.0d)).intValue();
    }
}
